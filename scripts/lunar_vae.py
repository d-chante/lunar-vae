...
def train(
	...
	for epoch in range(epochs):
		...
		epoch_val_l2_loss = 0
		epoch_val_kl_loss = 0
		epoch_val_elbo_loss = 0   # <<< added

		progress_bar = tqdm(val_loader, desc=f"[Validation] Epoch {epoch+1}/{epochs}")
		...
		with torch.no_grad():
			for x, _ in progress_bar:
				...
				epoch_val_elbo_loss += elbo_loss.item()   # <<< added
		...
		avg_val_elbo_loss = epoch_val_elbo_loss / len(val_loader)   # <<< added

		val_l1_losses.append(avg_val_l1_loss)
		val_l2_losses.append(avg_val_l2_loss)
		val_kl_losses.append(avg_val_kl_loss)
		val_elbo_losses.append(avg_val_elbo_loss)   # <<< added

		print(f"[Validation] Epoch {epoch+1}/{epochs}, "
			  f"Val L1: {avg_val_l1_loss:.4f}, "
			  f"Val L2: {avg_val_l2_loss:.4f}, "
			  f"Val KL: {avg_val_kl_loss:.4f}, "
			  f"Val ELBO: {avg_val_elbo_loss:.4f}")   # <<< changed

		if avg_val_elbo_loss < best_val_loss:   # <<< changed
			best_val_loss = avg_val_elbo_loss   # <<< changed
			SaveModel(vae, optimizer, scheduler, epoch, best_val_loss, model_dir)
	...
	return {
		...
		"val_elbo_losses": val_elbo_losses,   # <<< added
