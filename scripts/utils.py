...
def PlotRawAndInterpolatedData(profile_name, profile_dir_v2, profile_dir_v1=None, output_dir=None):
	with open(os.path.join(profile_dir_v2, profile_name), 'r') as file:
		data_v2 = json.load(file)
		
	if profile_dir_v1:
			with open(os.path.join(profile_dir_v1, profile_name)) as file:
				data_v1 = json.load(file)

	try:
		if profile_dir_v1:
			interpolated_data_v1 = data_v1["data"]
			interp_time_v1 = interpolated_data_v1["time"]
			interp_temps_v1 = interpolated_data_v1["temperature"]
		
		raw_data = data_v2["raw_data"]
		interpolated_data_v2 = data_v2["interpolated_data"]

		raw_time = raw_data["time"]
		raw_temps = raw_data["temps"]

		interp_time_v2 = interpolated_data_v2["time"]
		interp_temps_v2 = interpolated_data_v2["temps"]
	...
	if profile_dir_v1:
		plt.plot(
			...
			interp_temps_v1,
			'.',
			label='Interpolated Data (profiles-v1)',
			linewidth=1.5)
	plt.plot(
		interp_time_v2,
		interp_temps_v2,
		'-',
		label='Interpolated Data (profiles-v2)',
		linewidth=1.5)
	plt.xlim(0, 24)
	plt.xticks([0, 6, 12, 18, 24])
	plt.ylim(25, 425)

	plt.title(f'Comparison of Raw and Interpolated Data ({profile_name})')
	...
	if output_dir is not None:
		path = os.path.join(output_dir, f"plot_{profile_name.split('.')[0]}.png")
		plt.savefig(path)
